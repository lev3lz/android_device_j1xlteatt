...
{
    // error messages
    private static final String szERROR_BAD_PARAMETER = "Error: Bad parameter";
    private static final String szINVALID_CODE        = "is an invalid code";
    ...
    public static long encodeQP(InputStream input, ByteArrayOutputStream output) throws smlMimeException, IOException
    {
        ...
        while (true)
        {
            ...
            if ((current > ' ') && (current < 0x7F) && (current != '=') && (current != '(') && (current != ')') && (current != ':'))
            {
                // Printable chars
            ...
            else if ((current == CR))// || (current == LF))
            {
                ...
                //QP CRLF of encoding to match the server, it is not used again.
//                l_bufenc[linelen++] = CR;
//                l_bufenc[linelen++] = LF;
            ...
            else if ((current < ' ') || (current == '=') || (current >= 0x7F) || (current == '(') || (current == ')') || (current == ':'))
            {
                // Special Chars
                // output.write ((byte) '=');
